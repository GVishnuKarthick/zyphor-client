...
const loadConversations = async () => {
  try {
    const res = await fetch("https://zyphor-server-1.onrender.com/api/conversations", {
      headers: {
...
const loadUnreadCounts = async (convos) => {
  try {
    const res = await fetch("https://zyphor-server-1.onrender.com/api/messages/unread-count", {
      headers: {
        Authorization: `Bearer ${localStorage.getItem("zyphor_token")}`,
      },
    });
...
useEffect(() => {
  ...
  const loadMessages = async () => {
    try {
      const res = await fetch(`https://zyphor-server-1.onrender.com/api/messages/${selChat}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("zyphor_token")}`,
        },
      });
...
const handleAddStory = async (image) => {
  ...
  const handleViewProfile = async (username) => {
    ...
    try {
      const res = await fetch(`https://zyphor-server-1.onrender.com/api/users/${username}`, {
        headers: {
          Authorization: `Bearer ${localStorage.getItem("zyphor_token")}`,
        },
      });
